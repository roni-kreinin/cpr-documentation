...
The launch file will run all of the necessary [generators](../config/generators) to generate launch and parameter files for the simulation before launching Gazebo and spawning the robot. These will be placed under the setup path (defaults to `~/clearpath/`).
...
```
ros2 launch clearpath_gz simulation.launch.py setup_path:=/home/user/setup/path/
```
...
<center>
  <figure>
    <img
      src="/img/software_images/ros2/driving.gif"
      width="900"
    />
